...
分析：环形DP，设计状态时，用f[i,j,1]表示第i段时间为止，已睡去j个时间段，且第i段时间睡觉获得的最大效用。反之，f[i,j,0]表示上述状态下[i]第i段时间不睡觉能获得的最大效用值。
	  状态转移方程如下：
	  f[i,j,0]:=Max{f[i-1,j,0]，f[i-1,j,1]}
	  f[i,j,1]:=Max{f[i-1,j-1,0],f[i-1,j-1,1]+u[i]}
 	  上述方程从i=1开始顺推，推出的只能是第一段时间开始睡或者第一段时间没有睡的最大效用值，换句话说，就是无论如何没有把u[1]加入这个最大值中，但是题目要求有可能让我们把u[1]加入进去，所以只需考虑一下u[i]必被加入的情况。处理的方法就是，之前的初态为f[1,1,1]:=0;和f[1,0,0]:=0;其余f值均为不可行(即将第一秒睡不睡的值都记为0)，那么现在，我们只需把初态记为f[1,1,1]:=0;其余值均记为不可行，然后再将f[n,b,1]+u[1]与原来更新出来的的ans值作比较即可
*/
#include<iostream>
#include<cstdio>
#include<cstring>
